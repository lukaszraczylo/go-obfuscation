...
func readPclntabSection(binaryPath string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pclntab: failed to open ELF: %w", err)
	}
	defer f.Close()
...
func WriteCorruptedPclntab(binaryPath string, corrupted []byte, outputPath string) error {
	return fmt.Errorf("pclntab: offline ELF patching not yet implemented; use runtime corruption instead")
}
